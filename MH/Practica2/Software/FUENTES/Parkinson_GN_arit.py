...
def main (): 
	...
	for train_index,test_index in particiones: 
		...
		X_test=X[test_index]
		Y_test=Y[test_index]
		GN_arit(X_train,Y_train,0.3,0.5,0.7,0.001)

		break
	"""
	Parallel(n_jobs=num_cores)(delayed(GN_arit)(X[index[0]],Y[index[0]],0.3,0.5,0.7,0.001)for index in particiones )
